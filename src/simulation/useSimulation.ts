...
export function useSimulation() {
  ...
  const saves = useRef<{ id: number; resolve: (c: Checkpoint) => void; reject: (e: Error) => void; timeout: ReturnType<typeof setTimeout> }[]>([]), nextSaveId = useRef(0);
  ...
  useEffect(() => {
    ...
    w.onmessage = (event: MessageEvent<WorkerReply>) => {
      ...
      if (message.type === 'saved') { const index = saves.current.findIndex(pending => pending.id === message.id); if (index >= 0) { const [pending] = saves.current.splice(index, 1); clearTimeout(pending.timeout); pending.resolve(message.checkpoint); } return; }
  ...
  const checkpoint = useCallback(() => new Promise<Checkpoint>((resolve, reject) => {
    ...
    const request = { id: ++nextSaveId.current, resolve, reject, timeout: setTimeout(() => { saves.current = saves.current.filter(p => p !== request); reject(new Error('Saving timed out. Try restarting the worker.')); }, 10000) };
    saves.current.push(request); send({ type: 'save', id: request.id });
  }), [send]);
