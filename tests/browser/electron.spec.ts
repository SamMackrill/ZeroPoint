...
test('spin shells link 2D and 3D, alternate local turns and preserve display controls', async ({ page }) => {
  ...
  await expect(page.getByLabel('Full 3D shells', { exact: true })).toBeChecked();
  const sectionPair = page.getByRole('button', { name: 'Inspect section pair 2229', exact: true });
  await sectionPair.click();
  ...
  await expect(page.getByTestId('electron-pair-centre')).toHaveText('0.6000, 0, 0');
  await page.getByLabel('Spin turn magnification').selectOption('1');
  await page.getByRole('button', { name: 'Advance 1 τ' }).click();
  ...
  await page.getByLabel('Full 3D shells', { exact: true }).uncheck();
  await page.locator('.light-viewport-shell').screenshot({ path: 'test-results/spin-shells-cutaway.png' });
  await page.getByLabel('Visible spin shells').selectOption('4');
  ...
  await page.getByLabel('Adjacent shell preference').selectOption('shared');
  await expect(page.getByTestId('electron-spin-rate')).toHaveText('-0.88° / τ');
  await page.getByLabel('Electron spin axis').selectOption('x');
  await page.getByRole('button', { name: 'Apply electron parameters' }).click();
  await expect(page.locator('.electron-section header')).toContainText('Looking from +X');
  await page.getByLabel('Linked 2D section', { exact: true }).uncheck();
  ...
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await page.getByLabel('Visible spin shells').selectOption('2');
  await page.locator('.electron-linked-views').screenshot({ path: 'test-results/spin-shells-mobile.png' });
  await page.getByLabel('Local turn arrows', { exact: true }).uncheck();
